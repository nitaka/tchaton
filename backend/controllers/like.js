...
exports.like = (req, res, next) => {
    ...
    let sql = "SELECT * FROM tchaton.like;";
    pool.execute(sql, function (err, results) {
        ...
        if (!userLiked) {
            ...
            pool.execute(sql, [req.body.postId], function (err, result) {
                if (err) res.status(400).json({ err });
                let sql2 = `INSERT INTO tchaton.like (postId, userId) VALUES (?,?);`;
            ...
            pool.execute(sql,[req.body.postId], function (err, result) {
                if (err) res.status(400).json({ err });
                let sql2 = `DELETE FROM tchaton.like l WHERE l.postId = ? AND l.userId = ?`;
                pool.execute(sql2,[req.body.postId,req.body.userId], function (err, result) {
                    if (err) res.status(400).json({ err });
                    res.status(200).json(result)
                });
...
exports.liked = (req, res, next) => {
    // recupére les like
    let sql2 = `SELECT postId FROM tchaton.like WHERE userId = ?`;
