    ...
    <div class="container">
      <div v-if="currentUser" class="infoUser col-12">
        <div class="own" v-if="userId == currentUser.id">
          <label for="image">
            <img class="image__profil-change" v-bind:src="currentUser.imgProfil" alt="image Profile" srcset=""/>
            <img class="image__file" src="../assets/image-solid.svg" alt="Choisir image de profil" srcset=""/>
          </label>
          <input @change="modifImg" type="file" name="image" id="image" style="display: none"/>
        </div>
        <div v-else>
          <img class="imgProfFirst" v-bind:src="currentUser.imgProfil" alt="imgProf" srcset="" />
        </div>
        <div class="text-user">
      ...
      <div v-else class="noPosts">
        <p class="noPosts_text" v-if="currentUser">
          <strong>{{ currentUser.prenom }} {{ currentUser.nom }}</strong> n'a encore jamais publié de posts.
        </p>
      </div>
    </div>
  </div>
